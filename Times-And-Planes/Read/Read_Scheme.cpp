...
Read_Scheme(const string &name_of_file, vector<CheckPoint> &checkPoints, vector<Scheme> &schemes, vector<Cycle> &cycles)
	{
		 ...
		 * NameA (POINT1)(POINT12 POINT13)(LAND | 7): POINT1 POINT2 Str(POINT12 POINT13) POINT3 POINT4 POINT5 /Str POINT12
		 * 1) Название схемы [NameA]
		 * 2) Название точки начала схемы [POINT1]
		 * 3) Названия точек конца схемы [POINT12 POINT13]
		 * 4) Вспомогательная группа, посадочная метка (приписывается всем точкам этой схемы) либо количество повторений цикла
		 ...
		 * Если цикла нет, посадки тоже - 4 группа пустая
		...
		cmatch res;
		regex regular(
				R"((\w+)\s*\((\w+)\)\s*\(([\w\s]+)\)\s*(?:\((LAND|\d+)\)\s*)?:\s*([\w\s]+)?\s+(?:Str\(([\w\s]+)\)\s*([\w\s]+)\/Str)?\s*([\w\s]*)?)");
		...
		while (getline(SchemeFile, str))
		{
			...
			else
			{
				...
				cycles[k].finish = pointNameToID[res[3]];
				...
				{
					cerr << "Can't find '" << ex.what() << "' in line '" << i + k + 3 << "' among points from Points file"
						 << endl;
					exit(-2);
				}
